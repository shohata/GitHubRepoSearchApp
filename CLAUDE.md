...
**重要**: コードを変更した際は、必ず以下のコマンドを順番に実行し、全てのエラーが解消されるまで修正してください:
...
これらのコマンドは全て成功する必要があります。エラーが発生した場合は、エラーメッセージを確認し修正してから次のステップに進んでください。

#### チェックが必要な理由

- **format**: コードスタイルの一貫性を保ち、チーム開発での可読性を向上
- **lint**: 潜在的なバグや問題のあるコードパターンを検出
- **build**: 本番環境でのビルドエラーを事前に防止
